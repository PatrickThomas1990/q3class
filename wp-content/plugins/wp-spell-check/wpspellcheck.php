	...
	*/
	...
	function wpsc_set_global_vars() {
		...
		if (!isset($wpsc_settings) && sizeof($check_opt) != 0) {
			...
			$wpsc_settings = new SplFixedArray(148);
	...
	function add_menu() {	
		...
		add_submenu_page( 'wp-spellcheck.php', 'SEO Opportunities', 'SEO Opportunities', 'manage_options', 'wp-spellcheck-seo.php', 'wpsc_admin_empty_render');
	...
	function add_pro_menu() {
		...
		if (!$ent_included) {
			global $submenu;
			$permalink = 'https://www.wpspellcheck.com/purchase-options/';
			$submenu['wp-spellcheck.php'][] = array( 'Upgrade to Premium', 'manage_options', $permalink );
		} else {
			global $submenu;
			$permalink = 'https://www.wpspellcheck.com/account/';
    ...
    <script type="text/javascript">
        jQuery(document).ready( function($) {
            $( "ul#adminmenu a[href$='https://www.wpspellcheck.com/purchase-options/']" ).attr( 'target', '_blank' );
			$( "ul#adminmenu a[href$='https://www.wpspellcheck.com/purchase-options/']" ).css( 'color', '#EC8E1F' );
			$( "ul#adminmenu a[href$='https://www.wpspellcheck.com/purchase-options/']" ).css( 'font-weight', 'bold' );
	...
	function wpsc_finish_scan() {
		...
		if ($settings[45]->option_value != "Entire Site") return false;
			...
			if ($ent_included) {
				...
				//$loc = dirname(__FILE__)."/../../../debug.log";
				//$debug_file = fopen($loc, 'a');
				//$debug_var = fwrite( $debug_file, "Words Scanned: " . $settings[22]->option_value . "\r\n" );
				//$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
				//$debug_var = fwrite( $debug_file, "Spell Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds.\r\n\r\n" );
				//$debug_var = fwrite( $debug_file, "-------------------------6.3 Spell Check Scan Pro" . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
				//fclose($debug_file);
			} else {
				$end = round(microtime(true),5);
				//$loc = dirname(__FILE__)."/../../../debug.log";
				...
				//$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
				//$debug_var = fwrite( $debug_file, "Spell Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds.\r\n\r\n" );
				//$debug_var = fwrite( $debug_file, "-------------------------6.3 Spell Check Scan Base" . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
				//fclose($debug_file);
			}
	}
	...
	function wpsc_finish_empty_scan() {
		...
		if ($settings[100]->option_value == 'true') {
			...
			if ($ent_included) {
				...
				//$loc = dirname(__FILE__)."/../../../debug.log";
				//$debug_file = fopen($loc, 'a');
				//$debug_var = fwrite( $debug_file, "Fields Scanned: " . $settings[46]->option_value . "\r\n" );
				//$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
				//$debug_var = fwrite( $debug_file, "SEO Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds \r\n\r\n" );
				//$debug_var = fwrite( $debug_file, "-------------------------6.3 SEO Check Scan Pro" . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
				//fclose($debug_file);
			} else {
				$end = round(microtime(true),5);
				//$loc = dirname(__FILE__)."/../../../debug.log";
				...
				//$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
				//$debug_var = fwrite( $debug_file, "SEO Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds \r\n\r\n" );
				//$debug_var = fwrite( $debug_file, "-------------------------6.3 SEO Check Scan Base" . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
				//fclose($debug_file);
			}
		}
	}
