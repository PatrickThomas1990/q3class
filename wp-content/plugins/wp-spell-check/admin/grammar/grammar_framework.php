...
function wpgc_set_global_vars() {
	...
	if (sizeof($check_opt) != 0) {
		if (sizeof($wpgc_options) < 1) $wpgc_options = $wpdb->get_results("SELECT * FROM $options_table");
	}
}
...
function wpgc_create_meta_box( $post ) {
	...
	if (isset($_GET['wpgc-scan-page'])) { if ($_GET['wpgc-scan-page-grammar'] == "Gramme Check") wpgc_scan_individual($post->ID); }
	
	$score = $wpdb->get_results("SELECT * FROM $results_table WHERE page_id=$post->ID");
	if (sizeof($score) > 0) {
	?>
		<table border="0">
			<tr>
				<td style="padding: 5px 10px;">Grammar</td>
			</tr>
			<tr style="text-align: center;">
				<td style="padding: 0px 10px;"><?php echo $score[0]->grammar ?></td>
			</tr>
		</table>
	<?php
	} else { 
	?>
		<div>There is no grammar score for this post yet. Run a scan in order to get the score for this post.</div>
	<?php
	}
}
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				jQuery("#content-html").click(function(e) {
					...
					jQuery("#content_ifr").contents().find("#tinymce").contents().find(".hiddenSpellError").contents().unwrap();
					/*spelling_highlight.forEach(function (item) {
							jQuery(".wp-editor-area").html(jQuery(".wp-editor-area").html().replace(" &lt;span class='hiddenSpellError wpgc-complex' style='border-bottom: 2px solid #FFC0C0;'&gt;" + item + "&lt;/span&gt; ", " " + item + " "));
							jQuery("#content_ifr").contents().find("#tinymce").html(jQuery("#content_ifr").contents().find("#tinymce").html().replace(' <span class="hiddenSpellError wpgc-complex" style="border-bottom: 2px solid #59c033;" data-mce-style="border-bottom: 2px solid #FFC0C0;">' + item + '</span> ', " " + item + " "));
						});
...
function wpgc_finish_scan() {
	...
	if ($ent_included) {
		...
		//$loc = dirname(__FILE__)."/../../../../../debug.log";
		////////$debug_file = fopen($loc, 'a');
		////////$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
		////////$debug_var = fwrite( $debug_file, "Grammar Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds \r\n\r\n" );
		////////$debug_var = fwrite( $debug_file, "-------------------------6.3 Grammar Check Scan Pro " . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
		////////fclose($debug_file);
		
		$loc = "/mnt/stor19-wc2-dfw1/592545/609232/www.persyoonline.com/web/content/memory.log";
		////////$debug_file = fopen($loc, 'a');
		////////$debug_var = fwrite( $debug_file, "-------------------------6.3 Grammar Check Scan Pro " . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
		////////fclose($debug_file);
	} else {
		...
		////////$debug_var = fwrite( $debug_file, "Finalization Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: "  .round(memory_get_usage() / 1000,5)  . " \r\n" );
		////////$debug_var = fwrite( $debug_file, "Grammar Check Scan Complete. Total Time: " . ($end_time - $time) . " Seconds \r\n\r\n" );
		////////$debug_var = fwrite( $debug_file, "-------------------------6.3 Grammar Check Scan Base " . date('l F d g:i:sA') . "------------------------------\r\n\r\n\r\n" );
		////////fclose($debug_file);
