...
*/
	...
	function wpsc_print_debug($scan, $time, $sql, $memory, $error) {
		...
		//$debug_var = fwrite( $debug_file, "$scan     Time: $time.     SQL: $sql     Memory: $memory KB.     Errors: $error\r\n" );
		//fclose($debug_file);
	}
	...
	function wpsc_clean_text($content) {
		...
		$content = preg_replace("/[^a-zA-Z'’`éèùâêîôûçëïüáéíñóúüÁÉÍÑÓÊÚÜ¿¡]/iu", " ", $content);
	...
	function wpsc_divi_check($content) {
		...
		if ($divi_check->name == "Divi" || $divi_check->parent_name == "Divi") {
			global $wp_query;
	...
	function wpsc_clean_all($content, $wpsc_settings) {
		...
		$content = wpsc_clean_text($content);
	...
	function check_broken_code($rng_seed = 0, $is_running = false, $log_errors = true, $log_debug = true) {
		...
		for ($x = 0;$x < $page_list->getSize();$x++) {
			...
			if (sizeof($shortcode_errors) != 0) {
				foreach($shortcode_errors as $shortcode_error) {		
					if (isset($shortcode_error[0])) {
						$hold = new SplFixedArray(4);
		...
		if ($log_debug) wpsc_print_debug("Broken Code", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_broken_html($rng_seed = 0, $is_running = false, $log_errors = true, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Broken HTML", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_broken_shortcode($rng_seed = 0, $is_running = false, $log_errors = true, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Broken Shortcodes", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_pages($rng_seed = 0, $is_running = false, $wpsc_haystack = null, $log_errors = false, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Page Content", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_posts($rng_seed = 0, $is_running = false, $wpsc_haystack = null, $log_errors = false, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Post Content", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
function wpsc_check_author_spelling($wpsc_haystack = null, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Author", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
}
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Sitename", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
}
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Site Tagline", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Contact Form 7", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function email_admin() {
		...
		if (!$pro_included && !$ent_included) $output .= 'NOTE: You are using the free version of WP Spell check. <a href="https://www.wpspellcheck.com/purchase-options/">Upgrade</a> to Premium today to scan your entire site';
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Author SEO Title EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Author SEO Desc EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$loc = dirname(__FILE__)."/../../../../debug-var.log";
		//$debug_file = fopen($loc, 'a');
		//$debug_var = fwrite( $debug_file, "Post Content Ignore List: " . sizeof($ignore_list) . "          Dictionary List: " . sizeof($dict_list) . "          Options: " . sizeof($wpsc_settings) . "          Grammar Options: " . sizeof($wpgc_settings) . "\r\n" );
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Menus EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_page_title_free($is_running = false, $wpsc_haystack = null, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Page EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_post_title_free($is_running = false, $wpsc_haystack = null, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Post(End) EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Tag EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Category EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Seo Desc EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$dict_list = $wpdb->get_results("SELECT * FROM $dict_table;");
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("SEO Title EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$dict_list = $wpdb->get_results("SELECT * FROM $dict_table;");
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Slider Title EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$dict_list = $wpdb->get_results("SELECT * FROM $dict_table;");
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Slider Caption EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$dict_list = $wpdb->get_results("SELECT * FROM $dict_table;");
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Media EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("WooCommerce EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
...
$word_count = 0;
	...
	function check_broken_code_free($rng_seed = 0, $is_running = false, $log_debug = true) {
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Broken Code EPS", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
