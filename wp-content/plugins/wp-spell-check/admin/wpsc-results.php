...
class sc_table extends WP_List_Table {
	...
	function column_word($item) {
		...
		for ($x = 0; $x < sizeof($suggestions_holding); $x++ ) {
			$temp = '';
			$temp_per = 0;
			$temp_index = 0;
				for ($y = 0; $y < sizeof($suggestions_holding); $y++ ) {
	...
	function prepare_items() {
		...
		if (!empty($_REQUEST['orderby'])) usort($data, 'usort_reorder');
...
for ($x= 0; $x < sizeof($old_words); $x++) {
	...
	foreach($mass_edit as $edit_id) {
		if ($edit_id == $old_word_ids[$x] && !in_array($old_words[$x], $mass_edit_list)) {
	...
	if (strpos($word_list, ", ") !== false) {
		return $return_message . "The following words have been updated: " . $word_list;
	} else {
		if ($word_list != '') {
			return $return_message . "The following word has been updated: " . $word_list;
		} else {
...
function wpsc_mass_edit($to_update) {
	...
	foreach($to_update as $update) {
		foreach ($results as $result) {
			if ($update['old_word'] == $result->word) {
				if ($result->page_type == "Post Content" || $result->page_type ==  "Page Content" || $result->page_type == "Media Description" || $result->page_type == "WooCommerce Product" || $result->page_type == "WP eCommerce Product") {
					...
					$content = preg_replace("/(?<![A-Za-z0-9])" . $update['old_word'] . "(?![A-Za-z0-9])/",$update['new_word'],$content);
...
function wpsc_admin_render() {
	...
	if (!$ent_included && !$pro_included) $max_pages = 500;
	...
	/*$end = round(microtime(true),5);
	$loc = dirname(__FILE__)."/../../../../error-debug.log";
	$debug_file = fopen($loc, 'a');
	$debug_var = fwrite( $debug_file, "Init Finished(about to set time estimates if needed)     Time: " . round($end - $start,5) . ".      SQL: " . $sql_count . ".     Memory: " . round(memory_get_usage() / 1000,5) . " KB\r\n" );
	fclose($debug_file);*/
	...
	if (isset($_GET['action'])) {
		if ($_GET['action'] == 'check') {
			...
			$total_authors = sizeof($wpdb->get_results("SELECT * FROM $post_table GROUP BY post_author")); $sql_count++;
			...
			$total_seo_title = sizeof($wpdb->get_results("SELECT * FROM $postmeta_table WHERE meta_key='_yoast_wpseo_title' OR meta_key='_aioseop_title' OR meta_key='_su_title'")); $sql_count++;
			$total_seo_desc = sizeof($wpdb->get_results("SELECT * FROM $postmeta_table WHERE meta_key='_yoast_wpseo_metadesc' OR meta_key='_aioseop_description' OR meta_key='_su_description'")); $sql_count++;
			...
			$total_generic_slider = sizeof(get_pages(array('number' => PHP_INT_MAX, 'hierarchical' => 0, 'post_type' => 'slider', 'post_status' => array('publish', 'draft')))); $sql_count++;
  ...
  <p>Have you backed up your database? This will update all areas of your website that you have selected WP Spell Check to scan. Are you sure you wish to proceed with the changes?</p>
		...
		<div class="wrap wpsc-table">
			...
			<div class="wpsc-scan-nav-bar">
				...
				<a href="/wp-admin/admin.php?page=wp-spellcheck-seo.php" id="wpsc-empty-fields" name="wpsc-empty-fields">SEO Opportunities</a>
			...
			<form action="<?php echo admin_url('admin.php'); ?>" method='GET'>
				...
				<?php if ((($post_count + $page_count) > $max_pages) & $ent_included) echo "<h3 class='sc-message' style='color: rgb(225, 0, 0);'>You have more than $max_pages Pages/Posts. <a href='https://www.wpspellcheck.com/purchase-options' target='_blank'>Upgrade</a> to scan all of your website.</h3>" ?>
				<?php if (!$ent_included) {
					if ($word_count > 0 && $pro_words > 0) {
						echo "<h3 class='sc-message' style='color: rgb(225, 0, 0);'>" . $pro_words . " Errors were found on other parts of your website. <a href='https://www.wpspellcheck.com/purchase-options' target='_blank'>Upgrade</a> to find and fix all errors.</h3>";
					} else {
						//echo "<h3 class='sc-message' style='color: rgb(225, 0, 0);'><a href='https://www.wpspellcheck.com/purchase-options' target='_blank'>Upgrade</a> to scan all parts of your website.</h3>";
					}
				} ?>
				</div>
			</form>
