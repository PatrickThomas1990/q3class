...
*/
	...
	function check_page_title_empty($rng_seed, $is_running = false, $log_debug = true) {	
		...
		if (sizeof($wpsc_settings) < 1) wpsc_set_global_vars();
		...
		$page_list = SplFixedArray::fromArray($wpdb->get_results("SELECT post_title, ID FROM $page_table WHERE post_type='page'$post_status LIMIT 100"));
		...
		if ($log_debug) wpsc_print_debug("Empty Page Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_post_title_empty($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$posts_list = SplFixedArray::fromArray(get_posts(array('posts_per_page' => 100, 'post_type' => $post_type_list, 'post_status' => $post_status))); $sql_count++;
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Post Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function wpsc_check_author_empty($is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Author", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	}
	...
	function check_menus_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Menu", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_post_tag_descriptions_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Tag Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_post_categories_description_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Category Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_media_descriptions_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Media Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_media_captions_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Media Caption", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_media_alt_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Media Alt", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_woocommerce_name_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty WooCommerce Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_woocommerce_excerpt_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty WooCommerce Excerpt", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_wpecommerce_name_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty WPeCommerce Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_wpecommerce_excerpt_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty WPeCommerce Excerpt", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_author_seotitle_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Author SEO Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_author_seodesc_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Author SEO Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_yoast_page_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$seo_check = $wpdb->get_results('SELECT post_id, meta_value, meta_key FROM ' . $table_name . ' WHERE (meta_key="_yoast_wpseo_title" OR meta_key="_aioseop_title" OR meta_key="_su_title") GROUP BY post_id');
		
		foreach($seo_check as $value) {
			if ($value->meta_value == '') $haystack[$value->post_id] = "empty field";
		}
		
		
		$ignore_pages = $wpdb->get_results('SELECT keyword FROM ' . $ignore_table . ' WHERE type="page";');
		
		$seo_check = $wpdb->get_results('SELECT post_id, meta_value, meta_key FROM ' . $table_name . ' WHERE (meta_key="_yoast_wpseo_metadesc" OR meta_key="_aioseop_description" OR meta_key="_su_description") GROUP BY post_id');
		...
		$results = SplFixedArray::fromArray($wpdb->get_results("SELECT post_content, post_title, ID FROM $page_table WHERE post_type='page'$post_status LIMIT 5000"));
		...
		foreach($results as $desc) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				$desc_type = $seo_check[0]->meta_key;
				$error_count++;
				if ($desc_type == '_yoast_wpseo_metadesc') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Page Description', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc>post_title, 'page_type' => 'All in One SEO Page Description', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate Page SEO Description', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Page Description', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Page SEO Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_seo_titles_page_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		//$loc = "";
		////////////$debug_file = fopen($loc, 'a');
		////////////$debug_var = fwrite( $debug_file, "Page SEO Title Memory: " . round(memory_get_usage() / 1000,5) . "     Errors: " . sizeof($error_list) .  " KB\r\n" );
		////////////fclose($debug_file);
		...
		for($x = 0; $x < $results->getSize(); $x++) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				...
				if ($desc_type == '_yoast_wpseo_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Page Title', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'All in One SEO Page Title', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate SEO Page Title', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Page Title', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Page SEO Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_yoast_post_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$results = SplFixedArray::fromArray($wpdb->get_results("SELECT post_content, post_title, ID FROM $page_table WHERE post_type='post'$post_status LIMIT 5000"));
		...
		for($x = 0; $x < $results->getSize(); $x++) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				$desc_type = $seo_check[0]->meta_key;
				if ($desc_type == '_yoast_wpseo_metadesc') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Post Description', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc>post_title, 'page_type' => 'All in One SEO Post Description', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate Post SEO Description', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Post Description', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Post SEO Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_seo_titles_post_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		for($x = 0; $x < $results->getSize(); $x++) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				$desc_type = $seo_check[0]->meta_key;
				if ($desc_type == '_yoast_wpseo_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Post Title', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'All in One SEO Post Title', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate SEO Post Title', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Post Title', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Post SEO Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_yoast_media_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		$results = get_posts(array('posts_per_page' => 10000, 'post_type' => 'attachment'));
		...
		foreach($results as $desc) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				$desc_type = $seo_check[0]->meta_key;
				if ($desc_type == '_yoast_wpseo_metadesc') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Media Description', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc>post_title, 'page_type' => 'All in One SEO Media Description', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_description') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate Media SEO Description', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Media Description', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Media SEO Desc", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
	...
	function check_seo_titles_media_empty_free($rng_seed, $is_running = false, $log_debug = true) {	
		...
		foreach($results as $desc) {
			...
			if ($haystack[$desc->ID] == 'empty field') {
				...
				$error_count++;
				if ($desc_type == '_yoast_wpseo_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Yoast SEO Media Title', 'page_id' => $desc->ID));
				} elseif ($desc_type == '_aioseop_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'All in One SEO Media Title', 'page_id' => $desc->ID)); 
				} elseif ($desc_type == '_su_title') {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'Ultimate SEO Media Title', 'page_id' => $desc->ID)); 
				} else  {
					array_push($error_list, array('word' => "Empty Field", 'page_name' => $desc->post_title, 'page_type' => 'SEO Media Title', 'page_id' => $desc->ID)); 
				}
			}
		}
		
		$end = round(microtime(true),5);
		if ($log_debug) wpsc_print_debug("Empty Media SEO Title", round($end - $start,5), $sql_count, round(memory_get_usage() / 1000,5), sizeof($error_list));
