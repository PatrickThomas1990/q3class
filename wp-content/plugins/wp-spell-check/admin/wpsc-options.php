	...
	if ( $_POST['import'] == "Import Plugin Data" ) {
		...
		foreach($content as $item) {
			...
			if ($to_add == '[wpsc_settings]' && $item != '[wpsc_settings]' ) {
				...
				if (sizeof($settings) == 2) $wpdb->update($table_name, array('option_value' => $settings[1]), array('option_name' => $settings[0])); //Update the main settings table
			} elseif ($to_add == '[wpsc_grammar]' && $item != '[wpsc_grammar]' ) {
				$settings = explode("=", $item);
				if (sizeof($settings) == 2) $wpdb->update($grammar_table, array('option_value' => $settings[1]), array('option_name' => $settings[0])); //Update the grammar settings table
	...
	?>
		...
		<div class="wrap">
			...
			<table class="form-table wpsc-general-options-table" style="width: 75%; float: left; background: white;" cellpadding="10"><tbody>
				<tr><td scope="row" align="left" style="padding-top: 30px;"><label style="display: inline-block; margin-bottom: 6px;">API Key (For <a target="_blank" href="https://www.wpspellcheck.com/purchase-options/">Pro</a> Version)</label><br /><input type="text" name="api_key" value="<?php echo $api_key; ?>"></td>
...
<option value="monthly" <?php if ($scan_frequency_interval == 'monthly') echo "selected='selected'"; ?>>Month(s)</option>-->
			...
			<div id="wpsc-scan-options-tab" <?php if ($_POST['wpsc-scan-tab'] != 'scan') echo 'class="hidden"';?>>
				...
				<td scope="row" align="left"><input type="checkbox" name="check-authors" value="check-authors" <?php if ($check_authors == 'true') echo 'checked'; ?>>Authors</td></tr>
